...
export const StepFive = ({ form }: StepFiveProps) => {
  ...
  const [isAITextLoadinng, setIsAITextLoadinng] = useState(false);
  // Auto-generate About Me content based on form data
  const generateAboutMe = async () => {
    try {
      setIsAITextLoadinng(true);
      ...
      const aboutMeText = `I am a passionate ${professionType
        .join(", ")
        .toLowerCase()} with ${experience} years of experience in ${gamesPlayed}. My expertise lies in ${specialties}, till now i served ${numberOfClientServed} clients.
